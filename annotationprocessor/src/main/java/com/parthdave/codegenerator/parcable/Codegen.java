package com.parthdave.codegenerator.parcable;
...
import com.parthdave.codegenerator.facebooklogin.FacebookInitCodeTemplateGenerator;
...
@SupportedAnnotationTypes({"com.parthdave.annotations.AutoParcel", "com.parthdave.annotations.FacebookInit"})
@SupportedSourceVersion(RELEASE_7)
public final class Codegen extends AbstractProcessor {

    public final String TAG = "Codegen";
    ...
    public boolean process(Set<? extends TypeElement> set, RoundEnvironment roundEnvironment) {
        ...
        Collection<? extends Element> autoParcelAnnotatedElements = roundEnvironment.getElementsAnnotatedWith(AutoParcel.class);
        Collection<? extends Element> facebookInitAnnotatedElements = roundEnvironment.getElementsAnnotatedWith(FacebookInit.class);

        handleElementsCodeGeneration(autoParcelAnnotatedElements);
        handleElementsCodeGeneration(facebookInitAnnotatedElements);
       ...
       /* for (int index = 0; index < types.size(); index++) {
        ...
        for (TypeElement type : types) {
            ...
            if (type.getEnclosedElements() != null)
                processType(type, ElementFilter.methodsIn(type.getEnclosedElements()));
            else
                processType(type, null);

            System.out.println("type.getEnclosedElements():" + ElementFilter.methodsIn(type.getEnclosedElements()));
            if (ElementFilter.methodsIn(type.getEnclosedElements()) != null)
                for (ExecutableElement ee : ElementFilter.methodsIn(type.getEnclosedElements())) {
                    System.out.printf("%s Action value %s:%s\n", ee.getSimpleName(), "" + ee.getParameters(), "" + (ee.getParameters().size() > 0 ? ee.getParameters().get(0).asType() : ""));
                }
        }
    }


    private void processType(TypeElement type, List<ExecutableElement> executableElements) {
        String className = generatedSubclassName(type, 0);
        String source = generateClass(type, className, generatedSubClassName(type), executableElements, true);
    ...
    private String generateClass(TypeElement type, String className, String classToExtend, List<ExecutableElement> executableElements, boolean isFinal) {
        if (type == null) {
        ...
        TypeSpec.Builder subClass = TypeSpec.classBuilder(className)
                ...
                .addModifiers(FINAL)
                .addModifiers(PUBLIC)
                .superclass(classToExtendTypeName).addField(TypeName.INT, "version", PRIVATE).addMethod(FacebookInitCodeTemplateGenerator.getInstance(false).onCreate(executableElements));
    ...
    private MethodModel findMethods(List<ExecutableElement> executableElements, String onCreate) {
            ...
            if (executableElements.get(index).getSimpleName().toString().equals(onCreate)) {
                ...
                if (methodElement.getParameters() != null) {
                    ...
                    for (int paramIndex = 0; paramIndex < methodElement.getParameters().size(); paramIndex++) {
                        ...
                        for (int innerParamIndex = 0; index < variableElement.size(); index++) {

                            System.out.println(variableElement.get(innerParamIndex).getModifiers());
//                            params.add(ParameterSpec.builder(TypeName.VOID, String.valueOf(variableElement.get(innerParamIndex).getSimpleName()), variableElement.get(innerParamIndex).getModifiers()).build());
                        }
                    }
                }
